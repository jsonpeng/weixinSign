...
<div class="form-group col-sm-8">
    <div class="box box-solid">
        ...
        <div class="box-body">
            ...
            <?php $post_cats = app('zcjy')->CatRepo()->all(); ?>
            @if(count($post_cats))
                <div class="form-group">
                 
                    {!! Form::label('cat_name', '文案分类:') !!}

                    <select name="cat_name" class="form-control">
                        @foreach($post_cats as $cat)
                            <option value="{!! $cat->name !!}" @if(isset($post) && $post->cat_name == $cat->name) selected="selected" @endif>{!! $cat->name !!}</option>
                        @endforeach
                    </select>
